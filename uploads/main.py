#!C:\Users\13205\AppData\Local\Programs\Python\Python310\python.exe
...
def test_headerinject(url):
    rnd = randstring()
    try:
        r = pool.request('GET', url + '/%%0D%%0A%s:1' %
                         rnd, retries=False, redirect=False)
        if rnd in r.headers:
            pout("headerinject", url + '/%%0D%%0A%s:1' % rnd)
    except (urllib3.exceptions.HTTPError, UnicodeError,
            ConnectionRefusedError):
        pass
